...
//				client.filterWithHeader(Collections.singletonMap("Authorization", token));
	...
	public String insert(String index,String type,Object json){
		try {
			...
			IndexResponse response = client.prepareIndex(index, type).setSource(JSON.parseObject(source),XContentType.JSON).execute().actionGet();
	...
	public String update(String index,String type,String id,Object json){
		try {
			...
			String source = json instanceof String ?json.toString():JSON.toJSONString(json);
			UpdateResponse result = client.prepareUpdate(index, type, id).setDoc(JSON.parseObject(source),XContentType.JSON).execute().actionGet();
	...
	public String upsert(String index,String type,String id,Object json){
		try {
			...
			String source = json instanceof String ?json.toString():JSON.toJSONString(json);
			IndexRequest indexRequest = new IndexRequest(index, type, id).source(JSON.parseObject(source),XContentType.JSON);
			UpdateRequest updateRequest = new UpdateRequest(index, type, id).doc(JSON.parseObject(source),XContentType.JSON).upsert(indexRequest);              
